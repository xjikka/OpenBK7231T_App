...
#define BL0942_OPTBIT1_UART1 1
#define BL0942_OPTBIT2_UART2 2
...
static void ScaleAndUpdate(int adeviceindex, int auartindex, bl0942_data_t *data) {
...
static int BL0942_UART_TryToGetNextPacket(int adeviceindex, int auartindex) {
    ...
    ScaleAndUpdate(adeviceindex,auartindex,&data);
...
void BL0942_UART_Init(void) {
  if (!bl0942_opts) {
    ...
    if (bl0942_opts & BL0942_OPTBIT1_UART1) {
      BL0942_UART_InitEx(UART_PORT_INDEX_0);
    }
    if (bl0942_opts & BL0942_OPTBIT2_UART2) {
...
void BL0942_SPI_RunEverySecond(void) {
    ...
    ScaleAndUpdate(BL0942_DEVICE_INDEX_0,-1,&data);
}
