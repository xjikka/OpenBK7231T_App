...
energysensdataset_t datasetlist[BL_SENSDATASETS_COUNT] = {
  { {
      ...
      {{"current",		"A",		"Current",				"current",					"1",		},	3,			(float)0.002,		},	// OBK_CURRENT
      {{"power",			"W",		"Power",				"power",					"2",		},	2,			(float)0.25,		},	// OBK_POWER
      {{"apparent_power",	"VA",		"Apparent Power",		"power_apparent",			"9",		},	2,			(float)0.25,		},	// OBK_POWER_APPARENT
      {{"reactive_power",	"var",		"Reactive Power",		"power_reactive",			"10",		},	2,			(float)0.25,		},	// OBK_POWER_REACTIVE
      {{"power_factor",	"",			"Power Factor",			"power_factor",				"11",		},	2,			(float)0.05,		},	// OBK_POWER_FACTOR
      {{"energy",			UNIT_WH,	"Energy Total",			"energycounter",			"3",		},	3,			(float)0.1,		},	// OBK_CONSUMPTION_TOTAL
      {{"energy",			UNIT_WH,	"Energy Last Hour",		"energycounter_last_hour",	"4",		},	3,			(float)0.1,		},	// OBK_CONSUMPTION_LAST_HOUR
      //{{"",				"",			"Consumption Stats",	"consumption_stats",		"5",		},	0,			0,			},	// OBK_CONSUMPTION_STATS
      {{"energy",			UNIT_WH,	"Energy Today",			"energycounter_today",		"7",		},	3,			(float)0.1,		},	// OBK_CONSUMPTION_TODAY
      {{"energy",			UNIT_WH,	"Energy Yesterday",		"energycounter_yesterday",	"6",		},	3,			(float)0.1,		},	// OBK_CONSUMPTION_YESTERDAY
      {{"energy",			UNIT_WH,	"Energy 2 Days Ago",	"energycounter_2_days_ago",	"12",		},	3,			(float)0.1,		},	// OBK_CONSUMPTION_2_DAYS_AGO
      {{"energy",			UNIT_WH,	"Energy 3 Days Ago",	"energycounter_3_days_ago",	"13",		},	3,			(float)0.1,		},	// OBK_CONSUMPTION_3_DAYS_AGO
  ...
  if (asensdatasetix == BL_SENSORS_IX_0) { 
    ...
    if (NTP_IsTimeSynced()) {
      ...
      if (actual_mday != ltm->tm_mday)
      {
        ...
        //MQTT_PublishMain_StringFloat(sensdataset->sensors[OBK_CONSUMPTION_YESTERDAY].names.name_mqtt, BL_ChangeEnergyUnitIfNeeded(sensdataset->sensors[OBK_CONSUMPTION_YESTERDAY].lastReading ),
             ...
             (sensdataset->sensors[i].noChangeFrame >= changeSendAlwaysFrames) )
        {
            ...
            if (MQTT_IsReady() == true)
            {
				...
				if (i == OBK_CONSUMPTION_CLEAR_DATE) {
					...
					MQTT_PublishMain_StringFloat(sensdataset->sensors[i].names.name_mqtt, val, sensdataset->sensors[i].rounding_decimals, 0);
				}
				stat_updatesSent[asensdatasetix]++;
			}
        } else {
