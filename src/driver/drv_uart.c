...
int UART_GetSelectedPortIndex() {
  ...
  {
    return UART_PORT_INDEX_1;
  }
  else
  {
    return UART_PORT_INDEX_0;
  }
}

int UART_GetBufIndexFromPort(int aport) {
#if UART_BUF_CNT==2
  if (aport == 1) return UART_BUF_INDEX_1;
  else return UART_BUF_INDEX_0;
#else
...
int test_ty_read_uart_data_to_buffer(int port, void* param)
{
    int rc = 0;
  int cnt = 0;
